        ...
        GotWords result ->
            case result of
                Ok txt ->
                    ...
                    in
                    ( { model | words = ws, error = "", status = Loading }, Words.chooseRandomIndex ws)

                Err e ->
                    ( { model | status = Error, error = Types.httpErrorToString e }, Cmd.none)

        PickedIndex i ->
            case Types.getAt i model.words of
                Nothing ->
                    ( { model | status = Error, error = "Liste vide / index invalide." }, Cmd.none )

                Just w ->
                    ( { model | target = Just w, meanings = [], guess = "", error = "", status = Loading, showSolution = False }, Dictionary.fetchMeanings w )

        GotDefs result ->
            case result of
                Ok meanings ->
                    ( { model | meanings = meanings, status = Ready, error = "" }, Cmd.none)
